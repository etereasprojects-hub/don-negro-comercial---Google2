          ...
          <Card className="bg-slate-900 border-2 border-slate-800 shadow-2xl overflow-hidden">
            ...
            <CardContent className="p-6 space-y-5">
              <div className="space-y-2">
                ...
                <p className="text-[9px] text-slate-500 italic">Retorna categorías alteradas después de este momento.</p>
              </div>

              <div className="space-y-2">
                <Label className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Código Categoría (sku)</Label>
                <div className="relative">
          ...
          <Card className="bg-slate-900/50 border border-slate-800">
            <CardContent className="p-4 space-y-3">
               <div className="flex items-center gap-2 text-[10px] text-slate-600 font-black uppercase tracking-tighter">
                 <Info className="w-3 h-3" /> Propósito Técnico
               </div>
               <p className="text-[11px] text-slate-500 leading-relaxed italic">
                 Estas categorías corresponden a la organización lógica dentro del sistema contable y de stock de Fastrax, no necesariamente a la estructura visual de la web.
               </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-3 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl min-h-[120px] max-h-[120px] overflow-auto shadow-inner">
                <pre className="text-[10px] font-mono text-blue-400/90 leading-tight">
                  {lastPayload ? JSON.stringify(lastPayload, null, 2) : "// Configure filtros y ejecute consulta..."}
                </pre>
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl min-h-[120px] max-h-[120px] overflow-auto shadow-inner">
                <pre className="text-[10px] font-mono text-emerald-400/90 leading-tight">
                  {response ? JSON.stringify(response, null, 2) : "// No hay respuesta del servidor..."}
                </pre>
              </div>
            </div>
          </div>
                ...
                <Table>
                  <TableHeader className="bg-slate-950/30">
                    <TableRow className="border-slate-800">
                      <TableHead className="text-[10px] uppercase font-black text-slate-500 pl-8 w-[200px]">Código ERP (SKU)</TableHead>
                      <TableHead className="text-[10px] uppercase font-black text-slate-500">Descripción de Categoría</TableHead>
                    ...
                    {categories.length > 0 ? categories.map((cat, i) => (
                      <TableRow key={i} className="border-slate-800 hover:bg-slate-800/40 transition-colors group">
                        <TableCell className="pl-8">
                           <div className="flex flex-col">
                             <span className="font-mono text-amber-500 font-bold text-xs">#{cat.sku}</span>
                        ...
                        <TableCell>
                           <div className="flex items-center gap-3">
                             ...
                             <span className="text-sm font-bold text-white uppercase tracking-tight">{decodeSafe(cat.nom)}</span>
                           </div>
                        </TableCell>
                      ...
                      <TableRow>
                        <TableCell colSpan={3} className="text-center py-12 text-slate-600 italic text-sm">
                           No se han recuperado categorías del sistema. Inicie la sincronización.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
          ...
          {!loading && !response && (
            <div className="text-center py-28 bg-slate-900/20 rounded-[50px] border-4 border-dashed border-slate-800">
               ...
               <h3 className="text-slate-500 text-lg font-black uppercase tracking-tighter">Mapa del Sistema ERP</h3>
               <p className="text-slate-600 text-xs max-w-xs mx-auto mt-2 leading-relaxed font-bold uppercase italic">
                 Recupere la taxonomía de categorías para optimizar las consultas de productos por familias técnicas.
               </p>
            </div>
          )}
        </div>
