  ...
  return (
    <div className="bg-white h-screen p-4 max-w-[1400px] mx-auto overflow-hidden flex flex-col font-sans">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-[0.45fr_0.55fr] gap-10 min-h-0 pt-4 px-4">
        ...
        <div className="flex flex-col h-full space-y-6 min-h-0 justify-start">
          <div className="flex-[2.5] relative rounded-[2.5rem] overflow-hidden bg-slate-50/50 border border-slate-100 flex items-center justify-center min-h-0 group">
            ...
            {product.imagen_url ? (
              <img 
                ...
                alt={product.nombre} 
                className="max-w-[85%] max-h-[85%] object-contain relative z-10 drop-shadow-2xl"
              />
            ) : (
              ...
              <div className="grid grid-cols-2 gap-6">
                {config?.whatsapp_number && (
                  <div className="flex flex-col">
                    <span className="text-[7px] font-bold uppercase text-slate-400 mb-1 tracking-wider">Asesoría WhatsApp</span>
                    <span className="text-sm xl:text-base font-black text-[#2E3A52] tabular-nums leading-none tracking-tight">{config.whatsapp_number}</span>
                  </div>
                )}
                {config?.whatsapp_24_7 && (
                  <div className="flex flex-col">
                    <span className="text-[7px] font-bold uppercase text-slate-400 mb-1 tracking-wider">Soporte 24/7</span>
                    <span className="text-sm xl:text-base font-black text-[#2E3A52] tabular-nums leading-none tracking-tight">{config.whatsapp_24_7}</span>
                  </div>
                )}
                {locations.slice(0, 2).map(loc => (
                  <div key={loc.id} className="flex flex-col border-t border-slate-200/50 pt-2.5 text-left">
                    <div className="flex items-center gap-1.5 text-[7px] font-black uppercase text-slate-500 mb-1">
                      <MapPin className="w-2.5 h-2.5 text-[#D91E7A]" />
                      {loc.name}
                    </div>
                    <span className="text-[11px] font-black text-[#2E3A52] truncate leading-none">{loc.phone || 'Consulte en web'}</span>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <div className="h-full flex flex-col justify-between py-2 min-h-0 text-left">
          <div className="space-y-6 flex-1 flex flex-col min-h-0">
            ...
            <div className="space-y-3 flex-shrink-0">
              ...
              <h1 className="text-4xl lg:text-5xl xl:text-6xl font-black text-[#2E3A52] tracking-tighter uppercase leading-[0.9] lg:leading-[0.85] text-balance">
                {product.nombre}
              </h1>
            </div>
            ...
            <div className="space-y-3 flex-1 min-h-0 overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto pr-4 py-1">
                <p className="text-[#2E3A52]/80 text-sm lg:text-base xl:text-[1.05rem] leading-relaxed whitespace-pre-wrap font-medium">
                  {description}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4 pt-4 flex-shrink-0">
            <div className="relative group overflow-hidden bg-gradient-to-br from-[#D91E7A] to-[#6B4199] text-white p-5 rounded-[2.5rem] shadow-xl shadow-purple-200/50">
              <div className="relative z-10 flex flex-col items-center">
                <span className="text-[9px] font-black uppercase tracking-[0.3em] mb-1.5 text-white/70">OFERTA CONTADO EFECTIVO</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-xl lg:text-3xl font-black opacity-40 leading-none">₲</span>
                  <span className="text-4xl lg:text-5xl xl:text-6xl font-black tracking-tighter leading-none">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                ...
                <div key={idx} className="bg-slate-50/50 hover:bg-slate-100 transition-colors py-3 px-4 rounded-2xl border border-slate-100/50 text-center">
                  ...
                  <p className="text-lg font-black text-[#2E3A52] tracking-tighter tabular-nums leading-none">
